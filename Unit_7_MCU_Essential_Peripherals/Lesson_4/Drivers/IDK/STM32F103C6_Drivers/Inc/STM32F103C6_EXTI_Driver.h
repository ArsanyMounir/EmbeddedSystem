...
typedef struct
{
	EXTI_GPIO_Maping_t 		EXTI_Pin;		//Specifies the external interrupt GPIO Mapping
											//The parameter must be set based on @ref EXTI_Define

	uint8_t					Trigger_case;	//Specifies RISING or FALLING or BOTH
											//The parameter must be set based on @ref EXTI_Trigger_Define

	uint8_t					IRQ_EN;			//Enable or Disable the EXTI IRQ (This will enable the IRQ MASK in EXTI and NVIC
											//The parameter must be set based on @ref EXTI_IRQ_Define

	void(* Ptr_IRQ_CallBack)(void);			//Set the Function that will be called once the IRQ Occur
